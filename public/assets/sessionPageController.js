document.addEventListener('DOMContentLoaded', async () => {
  ...
  const [groups, sessions] = await Promise.all([
      fetch('/groups/get').then(r => r.json()),
      fetch('/sessions/weekly').then(r => r.json())
  ]);
  ...
  // Generate date headers (next 7 days)
  const headers = ['Group'];
  const dateHeaders = [];
  const today = new Date();
  today.setHours(0, 0, 0, 0);

  // Create date headers for the week
  for (let i = 0; i < 7; i++) {
      const date = new Date(today);
      date.setDate(today.getDate() + i);
      headers.push(date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' }));
      dateHeaders.push(new Date(date)); // Store Date objects
  }

  // Create table headers
  const headerRow = document.getElementById('scheduleHeaders');
  ...
  sessions.forEach(session => {
      const groupName = groupMap[session.group] || 'Unknown Group';
      if (!scheduleData[groupName]) scheduleData[groupName] = {};
      
      // Calculate session end date
      const sessionStart = new Date(session.startDate);
      const sessionEnd = new Date(sessionStart);
      sessionEnd.setMonth(sessionStart.getMonth() + session.months);

      // Generate all possible session dates
      const allSessionDates = [];
      let currentDate = new Date(sessionStart);
      
      while(currentDate <= sessionEnd) {
          const dayName = currentDate.toLocaleDateString('en-US', { weekday: 'long' });
          if(session.selectedDays.includes(dayName)) {
              allSessionDates.push(new Date(currentDate));
          }
          currentDate.setDate(currentDate.getDate() + 1);
      }

      // Find which of these dates fall in our target week
      allSessionDates.forEach(sessionDate => {
          // Check if this session date is in our target week
          dateHeaders.forEach((targetDate, columnIndex) => {
              if(sessionDate.toDateString() === targetDate.toDateString()) {
                  ...
                  scheduleData[groupName][columnIndex] = scheduleData[groupName][columnIndex] || [];
                  scheduleData[groupName][columnIndex].push(sessionInfo);
              }
          });
      });
  });
  ...
  // [Keep the same code for populating the table body and hover effects]

  // Populate table body
  const tbody = document.getElementById('scheduleBody');
  Object.entries(scheduleData).forEach(([groupName, days]) => {
    ...
    dateHeaders.forEach((_, index) => {
      ...
      const sessions = days[index] || [];
      
      if (sessions.length > 0) {
        cell.classList.add('session-cell');
        cell.innerHTML = `<div class="session-dot"></div>`;
        
        // Hover events
        cell.addEventListener('mousemove', (e) => {
          const tooltip = document.getElementById('sessionTooltip');
          tooltip.innerHTML = sessions.map(s => `
            <div class="session-info">
              <strong>${s.time}</strong> (${s.duration} mins)<br>
              Starts: ${new Date(s.startDate).toLocaleDateString()}<br>
              Duration: ${s.months} months
            </div>
          `).join('');
        ...
        cell.addEventListener('mouseleave', () => {
          document.getElementById('sessionTooltip').style.display = 'none';
        });
      }
  ...
  const tab1 = document.querySelector(".add-sessions-cover")
